...
#import "GAPWebUserAgent.h"
#import <WebKit/WebKit.h>

@implementation GAPWebUserAgent {
...
-(void)asyncRequest {
    ...
    GAPLog(@"async request User-Agent...");
...
-(void)syncResult {
    ...
    if (_semaphore) {
        ...
        GAPLog(@"waiting on user-agent thread");
        dispatch_semaphore_wait(_semaphore, timeout);
        GAPLog(@"waking up user-agent thread");
    }
}

-(void) getUserAgent {
    GAPLog(@"get UserAgent by WKWebView");
    ...
    [webView evaluateJavaScript:@"navigator.userAgent" completionHandler:^(id __nullable userAgent, NSError * __nullable error) {
        GAPLog(@"get UserAgent in %@ thread [%@]", NSThread.currentThread.isMainThread?@"Main":@"BG", userAgent);
